...
class VoiceCallwithBluetooth extends StatefulWidget {
  const VoiceCallwithBluetooth({Key? key}): super(key: key);
  @override
  State<StatefulWidget> createState() => _State();
}

class _State extends State<VoiceCallwithBluetooth> {
  final _users = <int>[];
  RtcEngine? _engine;
  bool isJoined = false;
  ...
  void _switchMicrophone() {
    ...
    _engine!.setDefaultAudioRoutetoSpeakerphone(openMicrophone);
  }

  void _muteAudioStream() {
    setState(() {
      muteAudio = !muteAudio;
    });
  ...
  Future<void> startAudioRecordingWithConfig(AudioRecordingConfiguration config) async {
  }
  ...
  // update the Firebase Firestore Database with token details
  Future<void> _updateDriver() {
    return FirebaseFirestore.instance.collection('callDB')
        .doc('theDriver')
        .update({'token': token, 'channelid': channelId});
  }
