...
import 'NoAnswer.dart';
import 'dart:io';
import 'package:path_provider/path_provider.dart';

class BasicVoiceCalltwo extends StatefulWidget {
  const BasicVoiceCalltwo({Key? key}): super(key: key);
  @override
  State<StatefulWidget> createState() => _State();
}

class _State extends State<BasicVoiceCalltwo> {
  final _users = <int>[];
  RtcEngine? _engine;
  bool isJoined = false;
    ...
    if (_start >= 35 && joinMessage == 'Ringing') {
      WidgetsBinding.instance!.addPostFrameCallback((_) {
        Navigator.of(context).pushReplacement(
            MaterialPageRoute(builder: (context) => const NoAnswer()));
      });
    }
  }
                  ...
                  Expanded(
                    child: FloatingActionButton(
                      ...
                      child: GestureDetector(
                          ...
                          const Icon(Icons.mic_off, color: Colors.green)
                      ),
                    ),
                  ),
                  Expanded(
